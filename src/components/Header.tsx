...
const Header = () => {
  ...
  return (
    ...
    >
      <div
        className={`flex flex-col sm:flex-row justify-around items-center bg-opacity-50 backdrop-blur-lg shadow-md p-1 relative`}
      >
        <Link to="/">
          <img
            className="h-15 w-15 hover:scale-110 transition-all duration-200"
            src={'/images/logo.png'}
            alt="Logo contenant les initiales EM"
          />
        </Link>
    ...
    `}
        ...
        <button
          type="button"
          onClick={toggleTheme}
          className={`cursor-pointer hidden md:flex items-center rounded p-2 bg-transparent ${isDarkMode ? 'hover:bg-[#020817]' : 'hover:bg-[#dad4c1]'} transition-all duration-200`}
        >
          {isDarkMode ? <Sun /> : <Moon />}
        </button>
        ...
        >
          ...
          <div className="flex justify-center mt-5">
            <button
              onClick={toggleTheme}
              type="button"
              className={`cursor-pointer items-center  rounded p-2 bg-transparent ${isDarkMode ? 'hover:bg-[#020817]' : 'hover:bg-[#dad4c1]'} transition-all duration-200`}
            >
              {isDarkMode ? <Sun /> : <Moon />}
            </button>
          </div>
